...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  const colorConfig = Object.entries(config).filter(
    ([, configItem]: [string, ChartConfig[string]]) =>
      (configItem.theme ?? configItem.color) !== undefined
  );
...
${colorConfig
  .map(([key, itemConfig]: [string, ChartConfig[string]]) => {
    const color = itemConfig.theme?.[theme as keyof typeof THEMES] ?? itemConfig.color ?? undefined;
        ...
        <div className="grid gap-1.5">
          {payload.map((rawItem, index) => {
            ...
            const indicatorColor: string | undefined =
              color ?? payloadFill ?? (typeof item.color === 'string' ? item.color : undefined);
