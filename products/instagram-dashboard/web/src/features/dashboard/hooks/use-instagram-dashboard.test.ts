...
import * as connection from '@/features/account/services/connection.service';
import * as dashboard from '@/features/dashboard/services/dashboard.service';
import type { DashboardData, GrowthMetric, GrowthPeriod } from '@/features/shared/types/instagram.types';


// The hook reads metrics and the connection's state — two modules now.
vi.mock('@/features/dashboard/services/dashboard.service');
vi.mock('@/features/account/services/connection.service');

/** Presented as one object, which is all the single service ever was here. */
const mockedService = { ...vi.mocked(dashboard), ...vi.mocked(connection) };
