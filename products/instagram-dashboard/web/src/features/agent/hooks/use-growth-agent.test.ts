...
import * as agentSettings from '@/features/agent/services/agent-settings.service'
import * as chat from '@/features/agent/services/chat.service'
import * as suggestions from '@/features/agent/services/suggestions.service'
import type { ChatMessage, ContentSuggestion, ChatResponse } from '@/features/shared/types/instagram.types'

// One module became four, so the mock follows. The hook talks to three of them.
vi.mock('@/features/agent/services/chat.service')
vi.mock('@/features/agent/services/suggestions.service')
vi.mock('@/features/agent/services/agent-settings.service')

/**
 * The tests were written against a single service object. Rather than rewrite
 * every call site, the three mocked modules are presented as one — which is all
 * `mockedService` ever was.
 */
const mockedService = {
  ...vi.mocked(chat),
  ...vi.mocked(suggestions),
  ...vi.mocked(agentSettings),
}
