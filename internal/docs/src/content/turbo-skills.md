...
title: Turbo-Repo
description: |
  Sistema de construcción de monorepos Turborepo. Se activa con: turbo.json, pipelines de tareas,
    dependsOn, caché, caché remota, CLI "turbo", --filter, --affected, optimización de CI, variables
    de entorno, paquetes internos, estructura/mejores prácticas de monorepos y límites (boundaries).

    Usar cuando el usuario: configure tareas/flujos/pipelines, cree paquetes, configure un
    monorepo, comparta código entre apps, ejecute paquetes modificados/afectados, depure caché,
    o tenga directorios apps/packages.
metadata:
...
// internal/api-example/package.json
...
1. **If dependencies ARE declared** (e.g., `"@core/types": "workspace:*"` in package.json), remove the `prebuild` script. Turbo's `dependsOn: ["^build"]` handles this automatically.
...
2. **If dependencies are NOT declared**, the `prebuild` exists because `^build` won't trigger without a dependency relationship. The fix is to:
   - Add the dependency to package.json: `"@core/types": "workspace:*"`
...
// WRONG - root turbo.json with many package-specific overrides
{
  "tasks": {
    "test": { "dependsOn": ["build"] },
    "@core/web#test": { "outputs": ["coverage/**"] },
    "@core/api#test": { "outputs": ["coverage/**"] },
    "@core/utils#test": { "outputs": [] },
    "@core/cli#test": { "outputs": [] },
    "@core/core#test": { "outputs": [] }
  }
}
